...
test('GET /once.txt should return 200 OK', async () => {
  ...
  try {
    const res1 = await fetch(`http://localhost:${ port }/once.txt`);

    expect(res1).toHaveProperty('ok', true);
    expect(await hasResolved(promises[0])).toBeTruthy();

    const res2 = await fetch(`http://localhost:${ port }/once.txt`);
...
test('OPTIONS /once.txt should keep return 200 OK until GET /once.txt', async () => {
  ...
  try {
    ...
    const res3 = await fetch(`http://localhost:${ port }/once.txt`);
...
test('GET should succeed with a strict ordered sequence', async () => {
  ...
  try {
    const res1 = await fetch(`http://localhost:${ port }/1.txt`);
    ...
    const res2 = await fetch(`http://localhost:${ port }/2.txt`);
    ...
    const res3 = await fetch(`http://localhost:${ port }/1.txt`);
...
test('GET should fail if out-of-order', async () => {
  ...
  try {
    const res1 = await fetch(`http://localhost:${ port }/2.txt`);

    expect(res1).toHaveProperty('status', 404);
    expect(await hasResolved(promises[0])).toBeFalsy();
    expect(await hasResolved(promises[1])).toBeFalsy();

    const res2 = await fetch(`http://localhost:${ port }/1.txt`);

    expect(res2).toHaveProperty('ok', true);
    expect(await hasResolved(promises[0])).toBeTruthy();
    expect(await hasResolved(promises[1])).toBeFalsy();

    const res3 = await fetch(`http://localhost:${ port }/2.txt`);

    expect(res3).toHaveProperty('ok', true);
    expect(await hasResolved(promises[0])).toBeTruthy();
    expect(await hasResolved(promises[1])).toBeTruthy();

    const res4 = await fetch(`http://localhost:${ port }/2.txt`);
...
test('GET unordered requests', async () => {
  ...
  try {
    ...
    const res2 = await fetch(`http://localhost:${ port }/1b.txt`);
    ...
    // 200: We got 1a
    const res4 = await fetch(`http://localhost:${ port }/1a.txt`);
    ...
    // 200: We got 2
    const res5 = await fetch(`http://localhost:${ port }/2.txt`);

    expect(res5).toHaveProperty('ok', true);
    expect(await hasResolved(promises[1])).toBeTruthy();

    // 404: Playbacks is finished
    const res6 = await fetch(`http://localhost:${ port }/2.txt`);
